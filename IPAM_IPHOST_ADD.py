...
if __name__ == '__main__':
    ...
    # print(InstanceData)  
    # load the json data , convert into python dictionary format    
    ...
    for ipaddress in ipcalc.Network(my_network):        
        ...
        nbip = requests.get(request_url,headers=headers)
        # print(type(nbip)) 
        ...
        print("IP : ",ipaddress)
        if netboxip['count'] == 0:
            print("IP not in netbox, will check in network",ipaddress)           
            # check if in network exists and not exists in netbox
            if ipaddress in found_ip_in_network:
                print("IPaddress found in Network :", ipaddress)           
                # print(ipaddress , Ins)             
                if str(ipaddress) in Ins:                  
                    # print("%s is found in JSON data" %ipaddress)
                    print("IP found in json, The HOSTNAME of IP", str(ipaddress),"is", Ins[str(ipaddress)])                     
                    # Adding in IP netbox            
                    netbox.ipam.create_ip_address(str(ipaddress),dns_name=Ins[str(ipaddress)])                  
                else:
                    netbox.ipam.create_ip_address(str(ipaddress),dns_name="N")
                    print(" IP Added in Netbox: ", str(ipaddress))                
                    
            else:
                pass
        else:
            print("IP exist in netbox",str(ipaddress))
